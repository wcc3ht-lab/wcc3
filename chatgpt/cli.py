...
def main():
    ...
    parser.add_argument(
        "--query", "-q", type=str, help="Single query to send to ChatGPT"
    )
    parser.add_argument(
        "--model",
        "-m",
    ...
    parser.add_argument(
        "--stream", "-s", action="store_true", help="Enable streaming responses"
    )
        ...
        try:
            if args.stream:
                ...
                for chunk in client.stream_chat(
                    args.query, temperature=args.temperature
                ):
        ...
        while True:
            try:
                ...
                if args.stream:
                    print("Assistant: ", end="", flush=True)
                    for chunk in client.stream_chat(
                        user_input, temperature=args.temperature
                    ):
                        print(chunk, end="", flush=True)
                    print()
                else:
